...
public struct SnowflakeId : IComparable<SnowflakeId>, IEquatable<SnowflakeId>
{
    ...
    public static bool operator ==(SnowflakeId lhs, SnowflakeId rhs) => lhs.Equals(rhs);

    public static bool operator !=(SnowflakeId lhs, SnowflakeId rhs) => !(lhs == rhs);

    public static SnowflakeId GenerateNewId() => GenerateNewId(GetTimestampFromDateTime(DateTime.UtcNow));
    ...
    public static byte[] Pack(int timestamp, int machine, short pid, int increment)
    {
        if ((machine & 0xff000000) != 0) throw new ArgumentOutOfRangeException(nameof(machine), @"The machine value must be between 0 and 16777215 (it must fit in 3 bytes).");
        if ((increment & 0xff000000) != 0) throw new ArgumentOutOfRangeException(nameof(increment), @"The increment value must be between 0 and 16777215 (it must fit in 3 bytes).");
        Console.WriteLine($"{timestamp} {machine} {pid} {increment}");
    ...
    private static int GetTimestampFromDateTime(DateTime timestamp) => (int)Math.Floor((ToUniversalTime(timestamp) - UnixEpoch).TotalSeconds);
    ...
    public int CompareTo(SnowflakeId other)
    {
        ...
        return r != 0 ? r : _increment.CompareTo(other._increment);
    }

    public bool Equals(SnowflakeId rhs) => _timestamp == rhs._timestamp && _machine == rhs._machine && _pid == rhs._pid && _increment == rhs._increment;

#pragma warning disable CS8765 // 参数类型的为 Null 性与重写成员不匹配(可能是由于为 Null 性特性)。
    public override bool Equals(object obj) => obj is SnowflakeId id && Equals(id);
    ...
    public static DateTime ToUniversalTime(DateTime dateTime) => dateTime == DateTime.MinValue
            ? DateTime.SpecifyKind(DateTime.MinValue, DateTimeKind.Utc)
            : dateTime == DateTime.MaxValue ? DateTime.SpecifyKind(DateTime.MaxValue, DateTimeKind.Utc) : dateTime.ToUniversalTime();
}