...
public static class IDCardValidation
{
    ...
    /// 18位身份证号码验证  
    /// </summary>  
    private static bool CheckIDCard18(string idno)
    {
    ...
    /// 15位身份证号码验证  
    /// </summary>  
    private static bool CheckIDCard15(string idno)
    {
        if (long.TryParse(idno, out var n) == false || n < Math.Pow(10, 14)) return false; //数字验证
        if (!address.Contains(idno.Remove(2))) return false; //省份验证  
        var birth = idno.Substring(6, 6).Insert(4, "-").Insert(2, "-");
