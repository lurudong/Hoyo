...
public static class ToLunar
{
    ...
    private static string FormatYear(int year)
    {
        ...
        while (year2 != 0)
        {
            _ = result.Append("零一二三四五六七八九".AsSpan(year2 % 10, 1));
            year2 /= 10;
        }
        return result.ToString().ReverseByPointer();
    }
    ...
    private static string FormatDay(int day)
    {
        ...
        return day switch
        {
            ...
            _ => day1 switch
            {
                0 => string.Concat("初", "十一二三四五六七八九".AsSpan(day2, 1)),
                1 => string.Concat("十", "十一二三四五六七八九".AsSpan(day2, 1)),
                2 => string.Concat("廿", "十一二三四五六七八九".AsSpan(day2, 1)),
                _ => throw new("不存在的农历日期"),
            },
        };
#endif
    ...
    private static bool Judge(IReadOnlyList<int> date)
    {
        ...
        var month = date[1];
        var day = date[2];
        return month is <= 12 and >= 1 && day is <= 31 and >= 1 && (day != 31 || (month != 2 && month != 4 && month != 6 && month != 9 && month != 11)) && (month != 2
            || day switch
            {
                > 29 => false,
